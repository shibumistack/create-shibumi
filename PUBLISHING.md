...
- Vendored clients current: `scripts/ship.lock.json` and `scripts/shibumi.lock.json` match the live immutable URLs; `bun run sync:ship` and `bun run sync:extensions` produce no diff.
