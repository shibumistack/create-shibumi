...
- The Ship client renders its setup plan only when stdin and stdout are both terminals, so the plan path cannot be tested through pipes. `test/fixtures/pty-run.py` owns a pty and feeds keystrokes in once the child goes quiet (`script` cannot do it: it needs its own stdin to be a terminal). Two gotchas it exists to encode: a pty forked with no window size reports zero columns and clack then wraps every single character onto its own line, and the agent-detection env vars of whatever shell runs the tests must be scrubbed (`humanEnv`) or the client takes its non-interactive path. `test/fixtures/fake-tools.ts` stands in for `gh` and `ssh`; its fake origin is a local bare repo wired in as the remote's *push* URL, because `git remote get-url` resolves `url.<base>.insteadOf` and the client refuses a non-GitHub origin.
...
- `git tag vX.Y.Z` fails "fatal: no tag message?": repo config forces annotated tags. Use `git tag -m "create-shibumi X.Y.Z" vX.Y.Z`.
- Generator attribution: the blog template's `<meta name="generator" content="Shibumistack.dev v{{generator-version}}">` token is substituted by `create.ts` from this package's `package.json` version at scaffold time, so generated projects always record the exact CLI release — nothing to update per release here. The separate shibumistack.dev repo can't read this package.json, so it tracks the same value in its own `package.json` under `createShibumiVersion`; bump that field in the site repo on every release, or the site's generator tag drifts.
- Blog dev (`bun --hot serve.ts`) and static preview (`bun scripts/preview.ts`) both bind 9001; a second instance fails loudly instead of silently binding the next port. Kill by port if needed (`lsof -ti :9001`). The blog renders live from source; nothing needs a build before dev.
