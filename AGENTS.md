...
- Astro 5 `astro dev`/`astro preview` (blog template) run as detached daemons: pkill by name misses them and the port stays owned; a second `preview` silently binds the NEXT port. Stop with `bunx astro dev stop` / `astro preview stop`, or kill by port (`lsof -ti :PORT`).
