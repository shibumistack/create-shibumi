...
export function applyAdd(root: string, ext: ExtensionBundle, plan: AddPlan): void {
  ...
  if (plan.migrationName && ext.migration) {
    // Exclusive create: a migration that appeared since planning fails loudly
    // instead of being overwritten.
    writeFileSync(join(root, MIGRATIONS_DIR, plan.migrationName), ext.migration, { flag: "wx" });
  }
  ...
  const guideTarget = join(root, agentsPath(ext));
  mkdirSync(dirname(guideTarget), { recursive: true });
  writeFileSync(guideTarget, ext.agentsFile);
...
export function planRemove(root: string, ext: ExtensionBundle): PlanResult<RemovePlan> {
  ...
  if (ext.migration && existsSync(migrationsDir)) {
    ...
    for (const candidate of readdirSync(migrationsDir)) {
      if (!new RegExp(`^\\d+_${ext.name}\\.sql$`).test(candidate)) continue;
  ...
  return { ok: true, plan: { deletions, modified, missing, edits, blockedEdits, migrationName, keptMigrations } };
}
...
export function applyRemove(root: string, ext: ExtensionBundle, plan: RemovePlan, force: boolean): void {
  ...
  if (plan.migrationName) {
    rmSync(join(root, MIGRATIONS_DIR, plan.migrationName), { force: true });
  }
...
export async function runCli(
  ...
  if ((result.plan.modified.length > 0 || result.plan.blockedEdits.length > 0) && !force) {
    io.err(
      "Some installed files or edits were modified after install (listed above). " +
