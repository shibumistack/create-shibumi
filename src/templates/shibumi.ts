 ...
 *
 * Flags: --yes (skip confirmation), --force (overwrite conflicting files on
 * add; on remove, also delete modified files and skip unreversible edits),
...
// inside the project root regardless.
...
export function applyAdd(root: string, ext: ExtensionBundle, plan: AddPlan): void {
  ...
  if (plan.migrationName && ext.migration) {
    writeFileSync(join(root, MIGRATIONS_DIR, plan.migrationName), ext.migration);
  }
...
export function planRemove(root: string, ext: ExtensionBundle): PlanResult<RemovePlan> {
  ...
  if (ext.migration && existsSync(migrationsDir)) {
    migrationName =
      readdirSync(migrationsDir).find((file) => new RegExp(`^\\d+_${ext.name}\\.sql$`).test(file)) ?? null;
  }

  return { ok: true, plan: { deletions, modified, missing, edits, blockedEdits, migrationName } };
}
...
export function applyRemove(root: string, ext: ExtensionBundle, plan: RemovePlan, force: boolean): void {
  ...
  if (existsSync(agentsMd)) {
    ...
    if (start !== -1 && end !== -1 && end > start) {
      ...
      writeFileSync(agentsMd, after.length > 0 ? `${before}\n${after}` : before);
    }
  }
}
...
export async function runCli(
  ...
  if (command === "add") {
      ...
      if (!interactive || yes) {
        io.err(
          `These files already exist: ${result.plan.conflicts.join(", ")}. ` +
            `Move them aside, or re-run with --force to overwrite.`
        );
        return 1;
      }
      const overwrite = await io.confirm(`Overwrite ${result.plan.conflicts.length} existing file(s)?`);
      if (!overwrite) {
    ...
    applyAdd(root, ext, result.plan);
    io.out(`${ext.title} installed. Run: bun test && bun run check`);
    if (Object.keys(result.plan.deps).length > 0) io.out("Dependencies were added; run: bun install");
    return 0;
  }
  ...
  applyRemove(root, ext, result.plan, force);
